...
<template lang="pug">
header(class="w-full p-2 flex items-center select-none h-[var(--HeaderHeight)] border-[var(--dark200)] border-b bg-white dark:bg-[var(--dark300)]" style="-webkit-app-region: drag;")
	...
	//- App Colors Theme
	ul(class="h-9 p-1 rounded-full flex items-center justify-center bg-[var(--dark200)] border-[var(--favColor)]" style="app-region: no-drag;")
	...
	//- Dark / Light Button
	div(class="toggleMode" style="-webkit-app-region: no-drag"  @click="darkModeChecked = !darkModeChecked; MT_toggleMode();")
...
.toggleMode {
	@apply h-8 w-16 mx-3 flex flex-row items-center justify-center rounded-full bg-[var(--dark100)] ;
	input {
		@apply w-0 h-0;
		visibility: hidden;
		&:checked + label {
